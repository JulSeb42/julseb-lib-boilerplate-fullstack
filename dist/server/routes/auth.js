...
router.post(PATHS.SIGNUP, (req, res, next) =>
	__awaiter(void 0, void 0, void 0, function* () {
		...
		if (
			!fullName ||
			!utils_1.emailRegex.test(email) ||
			!utils_1.passwordRegex.test(password)
		) {
			if (!fullName)
				res.status(400).json({
				...
				__awaiter(void 0, void 0, void 0, function* () {
					if (foundUser) {
						return res
							...
							.json({
								message:
									shared_1.COMMON_TEXTS.ERRORS.EMAIL_TAKEN,
							})
					}
					...
					return yield models_1.UserModel.create(
						...
						;(0, utils_2.sendMail)(
							email,
							shared_1.COMMON_TEXTS.EMAIL_SIGNUP_TITLE,
							shared_1.COMMON_TEXTS.EMAIL_SIGNUP_BODY(
								createdUser,
								verifyToken
							)
						)
						...
						const authToken = jsonwebtoken_1.default.sign(
							payload,
							utils_2.TOKEN_SECRET,
							utils_2.jwtConfig
						)
						return res
							.status(201)
							.json({ user: createdUser, authToken })
					})
				})
			)
	...
	__awaiter(void 0, void 0, void 0, function* () {
		...
		if (email === "" || password === "") {
			return res
				...
				.json({
					message:
						shared_1.COMMON_TEXTS.ERRORS.PROVIDE_EMAIL_AND_PASSWORD,
				})
		}
		return yield models_1.UserModel.findOne({ email })
			.then(foundUser => {
				if (!foundUser) {
					return res
						...
						.json({
							message:
								shared_1.COMMON_TEXTS.ERRORS.USER_NOT_EXIST,
						})
				}
				const passwordCorrect = bcryptjs_1.default.compareSync(
					password,
					foundUser.password
				)
				if (!passwordCorrect) {
...
router.put(PATHS.VERIFY(), (req, res, next) =>
	__awaiter(void 0, void 0, void 0, function* () {
		...
		yield models_1.UserModel.findById(id).then(foundUser =>
			__awaiter(void 0, void 0, void 0, function* () {
				if (!foundUser) {
					return res
						.status(400)
				...
				if (foundUser.verifyToken !== token) {
					return res
						...
						.json({
							message:
								shared_1.COMMON_TEXTS.ERRORS
									.VERIFY_TOKEN_NOT_MATCH,
						})
				}
				return yield models_1.UserModel.findByIdAndUpdate(
					id,
					{ verified: true },
					{ new: true }
				)
		...
		return yield models_1.UserModel.findOne({ email })
			.then(foundUser =>
				__awaiter(void 0, void 0, void 0, function* () {
					...
					return yield models_1.UserModel.findOneAndUpdate(
						{ email },
						{ resetToken },
						{ new: true }
					).then(foundUser => {
						...
						;(0, utils_2.sendMail)(
							email,
							shared_1.COMMON_TEXTS.EMAIL_RESET_PASSWORD_TITLE,
							shared_1.COMMON_TEXTS.EMAIL_RESET_PASSWORD_BODY(
								foundUser,
								resetToken
							)
						)
